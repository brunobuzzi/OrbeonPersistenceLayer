...
login
exec
| migrator application form |
migrator := OrbeonMySQLMigrator new.
application := migrator getOrbeonApplicationClass applicationNamed: '$2' ifNone: [^self].
form := application getDefinitionNamed: '$3' withVersion: $4 ifNone: [^self].
migrator migrateInstancesOf: form version: $4 from: '%5'.
System commit.
%
exit
EOF
